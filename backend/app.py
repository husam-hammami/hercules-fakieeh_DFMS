...
def create_order():
    ...
    order = {
        ...
        'sourceRawMaterialCode': data.get('sourceRawMaterialCode'),
...
def create_storage_bin():
    ...
    storage_bins.append(bin_obj)
    return jsonify(bin_obj), 201

@app.put('/api/storage-bins/<bin_id>')
def update_storage_bin(bin_id):
    data = request.get_json(force=True)
    for bin_obj in storage_bins:
        if bin_obj['id'] == bin_id:
            bin_obj.update({
                'name': data.get('name', bin_obj['name']),
                'materialCode': data.get('materialCode', bin_obj['materialCode']),
                'materialName': data.get('materialName', bin_obj['materialName']),
                'currentLevel': data.get('currentLevel', bin_obj['currentLevel']),
                'maxCapacity': data.get('maxCapacity', bin_obj['maxCapacity']),
                'status': data.get('status', bin_obj['status']),
                'highLevelFlag': data.get('highLevelFlag', bin_obj['highLevelFlag']),
                'lockLevelFlag': data.get('lockLevelFlag', bin_obj['lockLevelFlag']),
                'lastUpdated': datetime.utcnow().isoformat()
            })
            return jsonify(bin_obj)
    return jsonify({'error': 'not found'}), 404
...
@app.put('/api/bulk-transfers/<transfer_id>/status')
...
@app.put('/api/recipes/<recipe_id>/status')
def update_recipe_status(recipe_id):
    data = request.get_json(force=True)
    for recipe in recipes:
        if recipe['id'] == recipe_id:
            recipe['status'] = data.get('status', recipe['status'])
            recipe['actualQuantity'] = data.get('actualQuantity', recipe['actualQuantity'])
            recipe['endTime'] = data.get('endTime', recipe.get('endTime'))
